...
<html:form method="post" action="/storageManage.do?operate=shqdtoUpdate">
<div class="weizhi">
<div class="weizhi_bj"></div>
...
<table border="0" cellpadding="0" cellspacing="0" class="table_list">
	 <tr>
	    ...
	    <th width="89%">
		<div>
	      <input name="update" type="button" value="录入库存表" onclick="toUpdate();"/>
		</div>
		</th>
  	</tr>
	<tr>
							...
							<logic:notEmpty name="shqdlist">
								<logic:iterate id="item" name="shqdlist" indexId="indexNo">
									<tr onmouseover="this.className='over'" onmouseout="this.className='out'" onclick="showInfo(<bean:write name="indexNo"/>,this);">
										...
										<td><div><bean:write name="item" property="shdw" /></div></td>
										<td><div><bean:write name="item" property="pinzhongid" /></div></td>
...
<script language="JavaScript">
	function toAdd(){
		...
		document.storageForm.action="shqdManage.do?operate=toAdd";
		document.storageForm.submit();
	}
    function toUpdate()
    {
    	if(isRadioChecked(storageForm.selectNos)){
        document.storageForm.action="storageManage.do?operate=shqdtoUpdate";
        document.storageForm.submit();
        }
        else{
            alert("请选择唯一的品名！");
    ...
    function toDelete()
    {
   	 ...
   	 if(isRadioChecked(storageForm.selectNos))
        {
        	if(confirm("确定要删除吗？"))
        	 {
       document.storageForm.action="shqdManage.do?operate=toDelete";
        document.storageForm.submit();
        	}
        }
        else
