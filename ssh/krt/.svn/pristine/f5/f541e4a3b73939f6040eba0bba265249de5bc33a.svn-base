...
<html:form method="post" action="/supplier.do?operate=update">
<html:hidden property="customerBO.customerId"/>
<div class="weizhi">
	<div class="weizhi_bj">修改供应商信息</div>
</div>
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_add">
  ...
  <tr>
    ...
    <td width="25%"><bean:write name="customerBO" property="customerAccount"/>
	</td>     
	     <th width="5%">供应商名称：</th>
  ...
  <tr>
   ...
   <tr>
    ...
    <td><html:text property="customerBO.swdjzfddbr" styleId="Y_USN_customerBO.swdjzfddbr"  title="税务登记证： 法定代表人" maxlength="20"></html:text><span>*</span><font></font>
     ...
     <tr>
    <th>质量保证协议书有效期</th>
    <td><input type="text" name="customerBO.zlbzxysyxq" value="${customerBO.zlbzxysyxq}"  readonly="true" size="20" title="有效期至：" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="Wdate" /><font color="red">请选择确切日期</font>
	</td>
  </tr>
   <tr>
    <th>办事处</th>
    <td><html:text property="customerBO.banshichu" styleId="Y_str_customerBO.banshichu"  title="办事处" maxlength="20"></html:text><span>*</span>
	</td>
  </tr>
    <tr>
    <th>供应商级别</th>
  ...
  <tr>
    <th>vip：</th>
    <td>
		<html:radio property="customerBO.vip" value="1">是</html:radio>
		<html:radio property="customerBO.vip" value="0">否</html:radio>
	</td>
  </tr>
   ...
   function update()
    {
        if(checkSubmit(document.customerForm)){
        	if(confirm("确定要修改此信息吗？")){
	         	document.customerForm.action="supplier.do?operate=update";
				customerForm.submit();
	     	}
